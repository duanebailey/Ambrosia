...
class Image(AmbrosiaObject):
    ...
    presented (or kept) when images are shot.  If viewResult is True, ambrosia
    will attempt to display the image once rendered.  If keep is True, intermediate files
    ...
    def fileName(self,v=None):
        """Set value of fileName to v."""
        v = v or "untitled.png"
        (dir,base) = os.path.split(v)
        if dir:
            global environment
            environment.getProjectFolder(dir)
    ...
    def getFilePath(self):
        ...
        if self.getFileName() is None:
            ...
            return os.path.join(environment.getProjectFolder(),self.getFileName())
    ...
    def getFileNameSwitches(self):
        ...
        sw = " ".join([ks,ds,vsw])
        # if there is no file-path, we won't render.
        if fp is None:
            return ''
        if n is None:
            # no frame number: standard shoot
            return ' --output "{}" {}'.format(fp,sw)
        else:
            # frame number? advance the frame and shoot part of movie
            self.advanceFrame()
...
"""
    ...
    def present(self,imagePath):
        """Present the image in the platform-independent way."""
        if environment.getMedium() == 'jupyter':
            import IPython
            (w,h) = self.getImage().getDimensions()
    ...
    def buildMovie(self):
        ...
        i = self.getImage()
        pn = i.getFilePath() or userName or "untitled" ### DAB FIXME
        docmd(os.path.join(scriptsHome,'ambrosia-buildMovie') + ' --project "{}" --framecount {}'.format(pn,i.getFrameNumber()))
...
class StereoCamera(Camera):
    ...
    def _POV_shoot(self,*args):
        ...
        i._POV_shoot(*args)
        lf = pov.open(filebase+"-left.pov")
        self.pos(leftPos)
        self._POV_proof(*args)
        pov.close()
        rf = pov.open(filebase+"-right.pov")
        self.pos(rightPos)
        self._POV_proof(*args)
        pov.close()
        self.pos(nosePos)
        docmd(os.path.join(scriptsHome,"ambrosia-pov") + " --stereo"+fnsw+vsw)
...
class PIPCamera(Camera):
    ...
    def _POV_shoot(self,*args):
        ...
        # Are we missing an '.ini' file? DAB FIXME
        filebase = os.path.join(environment.projectdFolder(),userName)
        pov.open(filebase+"-large.pov")
        self._POV_proof(*args)
        pov.close()
        pov.open(filebase+"-small.pov")
        icam._POV_proof(*args)
        pov.close()
        docmd(os.path.join(scriptsHome,'ambrosia-pov')+' --pip --gravity "{}" {}{}'.format(self.getInsetPosition(),fnsw,vsw))
...
class IsometricCamera(Camera):
    ...
    def _POV_shoot(self,subject=None,*args):
        ...
        i._POV_shoot(*args)
        filebase = os.path.join(environment.getProjectFolder(),userName)
        pov.open(filebase+"-ortho.pov")
        self.pos(orthoPos)
        self._POV_proof(*args)
        pov.close()
        pov.open(filebase+"-right.pov")
        self.pos(rightPos)
        self._POV_proof(*args)
        pov.close()
        pov.open(filebase+"-top.pov")
        self.pos(topPos)
        self._POV_proof(*args)
        pov.close()
        pov.open(filebase+"-front.pov")
        self.pos(frontPos)
        self._POV_proof(*args)
        pov.close()
        # re-position camera where it was.
        self.pos(frontPos)
        # left in correct position
        docmd(os.path.join(scriptsHome,"ambrosia-pov") + " --isometric "+fnsw+vsw)
